...
#define BT_INVALID_ID 0xFFFFFFFF
typedef bt_s32 BT_KeyID;

#ifndef BT_CUSTOM_MEM
    #include <string.h>
    ...
    #define BT_BREAK_TRAP_ __buildin_trap()
#else
...
#define BT_MALLOC_SIG(name) void * name(void *user_context, size_t size)
typedef BT_MALLOC_SIG(bt_malloc_sig);

#define BT_FREE_SIG(name) void name(void *user_context, void *ptr)
typedef BT_FREE_SIG(bt_free_sig);

#define BT_REALLOC_SIG(name) void * name(void *user_context, void *ptr, size_t size)
typedef BT_REALLOC_SIG(bt_realloc_sig);
...
BT_API BT_ErrorCode
bt_create(BT_Context *tree, BT_Allocator *allocator);

BT_API BT_ErrorCode
bt_destroy(BT_Context *tree);

BT_API BT_Key *
bt_search(BT_Context *tree, bt_u32 id);

BT_API BT_ErrorCode
bt_insert(BT_Context *tree, bt_u32 id, const void *data);

BT_API BT_ErrorCode
bt_delete(BT_Context *tree, bt_u32 id);
...
BT_INTERNAL void *
bt_malloc(BT_Context *tree, size_t size);

BT_INTERNAL void
bt_free(BT_Context *tree, void *ptr);

BT_INTERNAL void *
bt_realloc(BT_Context *tree, void *ptr, size_t size);
...
bt_new_node(BT_Context *tree)
{
    BT_Node *node = (BT_Node *)bt_malloc(tree, sizeof(BT_Node));
    if (node != NULL) {
        node->key_count = 0;
...
bt_push_stack_frame(BT_Context *tree, BT_Node *node, BT_KeyID key_id)
{
    ...
    if (tree->frames_max == 0) {
        tree->frames_count = 0;
        tree->frames_max = 32;
        tree->frames = (BT_StackFrame *)bt_malloc(tree, sizeof(BT_StackFrame) * tree->frames_max);
        if (tree->frames == NULL) {
    ...
    if (tree->frames_count >= tree->frames_max) {
        tree->frames_max *= 2;
        tree->frames = (BT_StackFrame *)bt_realloc(tree, tree->frames, sizeof(BT_StackFrame) * tree->frames_max);
        if (tree->frames == NULL) {
            return BT_ERROR_AllocationFailed;
        }
    }
...
bt_delete(BT_Context *tree, bt_u32 id)
{
    ...
    bt_u32 key_index_delete = BT_INVALID_ID;
